...
export const mixChainPrompt: PromptDefinition = {
  ...
  handler: ({ source = "drums", track_index = "0" }) => {
    ...
    const recommendations: Record<string, string[]> = {
      drums: [
        "1. `apply_recipe { recipe_id: 'racks/parallel-comp', overrides: { track_index: " +
          idx +
          " } }`",
      ...
      bass: [
        "1. `apply_recipe { recipe_id: 'mixing/bass-glue', overrides: { track_index: " +
          idx +
          " } }`",
        "2. Sidechain do kick: `apply_recipe { recipe_id: 'racks/sidechain-rack', overrides: { track_index: " +
          idx +
          " } }`",
      ],
      vocal: [
        "1. `apply_recipe { recipe_id: 'mixing/vocal-chain', overrides: { track_index: " +
          idx +
          " } }`",
      ...
      messages: [
        {
          ...
          content: {
            ...
            text: [
              ...
              "",
              "Call `device_get_parameters { track_index: " +
                idx +
                ", device_index: N }` to discover what's loadable.",
            ].join("\n"),
          },
        },
      ],
