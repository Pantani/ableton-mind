      ...
      messages: [
        {
          ...
          content: {
            ...
            text: [
              ...
              "",
              `2. **List params**: \`device_get_parameters { track_index: ${track_index}, device_index: 0 }\`. Knowledge enriches them with unit/description.`,
              "",
              `3. **Starting point for ${target}:**`,
              ...tips.map((t) => `   - ${t}`),
              "",
              "4. **Tweak loop:**",
              "   - `device_set_parameter` by name (`parameter_name`) is more ergonomic than using an index.",
              '   - Watch `verified` + `diff` in the response. Curve params (`unit: "curve"`): use 0.05 steps.',
              "",
              "5. **Verify by ear:** create a test MIDI clip with 2-3 notes (`clip.create_midi` + `clip.add_notes`), then `clip.fire` and listen.",
              "",
              "6. **Iterate.** If the sound does not match the intent, adjust the key params again. `session_snapshot` before and `session_diff` after show exactly what changed.",
            ].join("\n"),
          },
        },
      ],
