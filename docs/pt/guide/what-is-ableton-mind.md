...
O codigo ja passou smoke real no Ableton Live 12.4.1 para operacoes centrais de transport/session/track, e os gates de package validation do `0.1.1` estao verdes. npm, GitHub Release `.mcpb` e MCP Registry estao publicados. Glama tem listagem, mas o release hospedado e publicado por um fluxo admin separado da Glama; use instalacao via source quando precisar desenvolver ou validar mudancas locais.
