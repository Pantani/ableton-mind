...
The code has passed a real Ableton Live 12.4.1 smoke test for core transport/session/track operations, and the `0.1.0` package validation gates are green. npm, GitHub Release `.mcpb` and MCP Registry channels are published. Glama has a listing, but its hosted release is published through a separate Glama admin flow; use source install when you need to develop or validate local changes.
